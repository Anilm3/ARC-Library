...
arc_hkey_t arc_hash_pearson(const void *data, size_t data_size)
{
    size_t i, j;
    arc_hkey_t hkey = 0;
    const unsigned char * data_bytes = data;
    ...
    for (j = 0; j < sizeof(arc_hkey_t); j++)
    {
        unsigned char h = T[(data_bytes[0] + j) % 256];
...
{
    size_t i;
    arc_hkey_t hkey = 5381;
    const unsigned char * data_bytes = data;

    for (i = 0; i < data_size; i++)
    {
        hkey = ((hkey << 5) + hkey) + data_bytes[i]; /* hash * 33 + c */
    }
