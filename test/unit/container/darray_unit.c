...
ARC_UNIT_TEST(empty)
{
    ...
    ARC_ASSERT_INT_EQ(arc_darray_push_front(darray, (void *)&i), ARC_SUCCESS);
...
ARC_UNIT_TEST(size)
{
    ...
    ARC_ASSERT_INT_EQ(arc_darray_size(darray), 1);
    ...
    ARC_ASSERT_INT_EQ(arc_darray_size(darray), 11);
    ...
    ARC_ASSERT_INT_EQ(arc_darray_size(darray), 6);
...
ARC_UNIT_TEST(push_pop_front_front)
{
    ...
    {
        ARC_ASSERT_INT_EQ(*((int *)arc_darray_front(darray)), i--);
...
ARC_UNIT_TEST(push_pop_back_back)
{
    ...
    for (i = 0; i < 20000; i++)
    {
        ARC_ASSERT_INT_EQ(arc_darray_push_back(darray, (void *)&i), ARC_SUCCESS);
    }

    i = 19999;
    while(!arc_darray_empty(darray))
    {
        ARC_ASSERT_INT_EQ(*((int *)arc_darray_back(darray)), i--);
...
ARC_UNIT_TEST(indexed_access)
{
    ...
    for (i = 0; i < 20000; i++)
    {
        ARC_ASSERT_INT_EQ(*((int *)arc_darray_at(darray, (unsigned)i)), i);
    }
...
ARC_UNIT_TEST(iterators_forward)
{
    int i;
    arc_darray_t darray = arc_darray_create(sizeof(int));
    arc_darray_iterator_t it = arc_darray_iterator_create(darray);
    ...
    {
        ARC_ASSERT_INT_EQ(*((int *)arc_darray_data(it)), i--);
    }

    arc_darray_iterator_destroy(it);
    arc_darray_destroy(darray);
}
...
ARC_UNIT_TEST(iterators_position)
{
    ...
    for (i = 0; i < 20000; i++)
    {
        ARC_ASSERT_INT_EQ(arc_darray_push_back(darray, (void *)&i),
                             ARC_SUCCESS);
    }

    for (i = 0; i < 20000; i++)
    {
        ARC_ASSERT_INT_EQ(arc_darray_position(it, i), ARC_SUCCESS);
        ARC_ASSERT_INT_EQ(*((int *)arc_darray_data(it)), i);
    }
...
ARC_UNIT_TEST(iterators_insertion_front)
{
    ...
    ARC_ASSERT_INT_EQ(arc_darray_insert_before(it, (void *)&i), ARC_ERROR);

    for (i = 0; i < 20000; i++)
    {
        ARC_ASSERT_INT_EQ(arc_darray_insert_after(it, (void *)&i), 
                             ARC_SUCCESS);
    }
...
ARC_UNIT_TEST(iterators_insertion_back)
{
    ...
    ARC_ASSERT_INT_EQ(arc_darray_insert_after(it, (void *)&i), ARC_ERROR);

    for (i = 0; i < 20000; i++)
    {
        arc_darray_after_end(it);
        ARC_ASSERT_INT_EQ(arc_darray_insert_before(it, (void *)&i), 
                             ARC_SUCCESS);
    }
...
ARC_UNIT_TEST(iterators_insertion_middle)
{
    ...
    /* 0 2 4 6 8 */
    i = 1;
    ARC_ASSERT_INT_EQ(arc_darray_position(it, 0), ARC_SUCCESS);
    ARC_ASSERT_INT_EQ(arc_darray_insert_after(it, (void *)&i), ARC_SUCCESS);

    /* 0 1 2 4 6 8 */
    i = 3;
    ARC_ASSERT_INT_EQ(arc_darray_position(it, 3), ARC_SUCCESS);
    ARC_ASSERT_INT_EQ(arc_darray_insert_before(it, (void *)&i), ARC_SUCCESS);

    /* 0 1 2 3 4 6 8 */
    i = 5;
    ARC_ASSERT_INT_EQ(arc_darray_position(it, 4), ARC_SUCCESS);
    ARC_ASSERT_INT_EQ(arc_darray_insert_after(it, (void *)&i), ARC_SUCCESS);

    /* 0 1 2 3 4 5 6 8 */
    i = 7;
    ARC_ASSERT_INT_EQ(arc_darray_position(it, 7), ARC_SUCCESS);
    ARC_ASSERT_INT_EQ(arc_darray_insert_before(it, (void *)&i), ARC_SUCCESS);

    /* 0 1 2 3 4 5 6 7 8 */
    i = 9;
    ARC_ASSERT_INT_EQ(arc_darray_position(it, 8), ARC_SUCCESS);
    ARC_ASSERT_INT_EQ(arc_darray_insert_after(it, (void *)&i), ARC_SUCCESS);
    ...
    for (i = 0; i < 20000; i++)
    {
        ARC_ASSERT_INT_EQ(*((unsigned *)arc_darray_at(darray, i)), i);
    }
...
ARC_UNIT_TEST(iterators_erase)
{
    ...
    {
        ARC_ASSERT_INT_EQ(*((unsigned *)arc_darray_at(darray, i - 10000)), i);
    }
    ...
    for (i = 0; i < 10000; i++)
    {
        ...
        ARC_ASSERT_INT_EQ(arc_darray_position(it, end_idx), ARC_SUCCESS);
        arc_darray_erase(it);
    }
    ...
    for (i = 20000; i > 0; i-=2)
    {
        ARC_ASSERT_INT_EQ(arc_darray_position(it, i - 1), ARC_SUCCESS);
        arc_darray_erase(it);
    }

    for (i = 0; i < 10000; i++)
    {
        ARC_ASSERT_INT_EQ(*((unsigned *)arc_darray_at(darray, i)), i*2);
    }
...
ARC_UNIT_TEST(destruction)
{
    ...
    {
        double data = i;
        ARC_ASSERT_INT_EQ(arc_darray_push_front(darray, (void *)&data), 
                             ARC_SUCCESS);
    }
