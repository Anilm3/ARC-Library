...
ARC_UNIT_TEST(iterators_forward)
{
    ...
    arc_deque2_before_begin(it);

    while(arc_deque2_next(it))
    {
        ARC_ASSERT_INT_EQ(*((int *)arc_deque2_data(it)), i--);
    }
...
ARC_UNIT_TEST(iterators_backward)
{
    ...
    arc_deque2_after_end(it);

    while(arc_deque2_previous(it))
    {
        ARC_ASSERT_INT_EQ(*((int *)arc_deque2_data(it)), i++);
    }
...
ARC_UNIT_TEST(iterators_position)
{
    ...
    for (i = 0; i < 20000; i++)
    {
        ARC_ASSERT_INT_EQ(arc_deque2_push_back(deque, (void *)&i),
                             ARC_SUCCESS);
    }

    for (i = 0; i < 20000; i++)
    {
        ARC_ASSERT_INT_EQ(arc_deque2_position(it, i), ARC_SUCCESS);
        ARC_ASSERT_INT_EQ(*((int *)arc_deque2_data(it)), i);
    }
...
ARC_UNIT_TEST(iterators_insertion_front)
{
    ...
    arc_deque2_before_begin(it);
    ARC_ASSERT_INT_EQ(arc_deque2_insert_before(it, (void *)&i), ARC_ERROR);

    for (i = 0; i < 20000; i++)
    {
        arc_deque2_before_begin(it);
        ARC_ASSERT_INT_EQ(arc_deque2_insert_after(it, (void *)&i), 
                             ARC_SUCCESS);
    }
...
ARC_UNIT_TEST(iterators_insertion_back)
{
    ...
    ARC_ASSERT_INT_EQ(arc_deque2_insert_after(it, (void *)&i), ARC_ERROR);

    for (i = 0; i < 20000; i++)
    {
        arc_deque2_after_end(it);
        ARC_ASSERT_INT_EQ(arc_deque2_insert_before(it, (void *)&i), 
                             ARC_SUCCESS);
    }
...
ARC_UNIT_TEST(iterators_insertion_middle)
{
    ...
    /* 0 2 4 6 8 */
    i = 1;
    ARC_ASSERT_INT_EQ(arc_deque2_position(it, 0), ARC_SUCCESS);
    ARC_ASSERT_INT_EQ(arc_deque2_insert_after(it, (void *)&i), ARC_SUCCESS);

    /* 0 1 2 4 6 8 */
    i = 3;
    ARC_ASSERT_INT_EQ(arc_deque2_position(it, 3), ARC_SUCCESS);
    ARC_ASSERT_INT_EQ(arc_deque2_insert_before(it, (void *)&i), ARC_SUCCESS);

    /* 0 1 2 3 4 6 8 */
    i = 5;
    ARC_ASSERT_INT_EQ(arc_deque2_position(it, 4), ARC_SUCCESS);
    ARC_ASSERT_INT_EQ(arc_deque2_insert_after(it, (void *)&i), ARC_SUCCESS);

    /* 0 1 2 3 4 5 6 8 */
    i = 7;
    ARC_ASSERT_INT_EQ(arc_deque2_position(it, 7), ARC_SUCCESS);
    ARC_ASSERT_INT_EQ(arc_deque2_insert_before(it, (void *)&i), ARC_SUCCESS);

    /* 0 1 2 3 4 5 6 7 8 */
    i = 9;
    ARC_ASSERT_INT_EQ(arc_deque2_position(it, 8), ARC_SUCCESS);
    ARC_ASSERT_INT_EQ(arc_deque2_insert_after(it, (void *)&i), ARC_SUCCESS);
    ...
    for (i = 0; i < 20000; i++)
    {
        ARC_ASSERT_INT_EQ(*((unsigned *)arc_deque2_at(deque, i)), i);
    }
...
ARC_UNIT_TEST(iterators_erase)
{
    ...
    for (i = 100; i < 200; i++)
    {
        ARC_ASSERT_INT_EQ(*((unsigned *)arc_deque2_at(deque, i - 100)), i);
    }
    ...
    for (i = 0; i < 10000; i++)
    {
        unsigned long end_idx = arc_deque2_size(deque) - 1;
        ARC_ASSERT_INT_EQ(arc_deque2_position(it, end_idx), ARC_SUCCESS);
        arc_deque2_erase(it);
    }
    ...
    for (i = 20000; i > 0; i-=2)
    {
        ARC_ASSERT_INT_EQ(arc_deque2_position(it, i - 1), ARC_SUCCESS);
        arc_deque2_erase(it);
    }

    for (i = 0; i < 10000; i++)
    {
        ARC_ASSERT_INT_EQ(*((unsigned *)arc_deque2_at(deque, i)), i*2);
    }
    arc_iterator_destroy(it);
    arc_deque2_destroy(deque);
}
